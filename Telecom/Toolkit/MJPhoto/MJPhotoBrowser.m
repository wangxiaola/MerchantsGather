...
- (void)createToolbar
{
    ...
    if (self.isDelete == NO){
        ...
        UIButton *btu =[[UIButton alloc]initWithFrame:CGRectMake(self.view.frame.size.width-50, self.view.frame.size.height-34, 20, 20)];
        [btu setBackgroundImage:[UIImage imageNamed:@"delete"] forState:UIControlStateNormal];
        [btu setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [btu setTitleColor:[UIColor grayColor] forState:UIControlStateHighlighted];
        [btu addTarget:self action:@selector(dexdelete) forControlEvents:UIControlEventTouchUpInside];
...
-(void)dexdelete
{
    ...
    // NSLog(@" h %lu %ld",(unsigned long)self.photos.count,(long)_currentPhotoIndex);
    
    if ([self.delegate respondsToSelector:@selector(index:)]) {
        [self.delegate index:_currentPhotoIndex];
    }
      _currentPhotoIndex =self.photos.count-1;
    
  // NSLog(@"  g %lu %ld",(unsigned long)self.photos.count,(long)_currentPhotoIndex);
