...
{
    [self insertArrayByAsyncToDB:models completed:^(BOOL completed) {
...
+ (void)insertArrayByAsyncToDB:(NSArray *)models completed:(void (^)(BOOL))completedBlock
{
    if (models.count > 0) {
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            [self insertToDBWithArray:models filter:^(id model, BOOL inserted, BOOL *rollback) {
...
{
    [self insertToDBWithArray:models filter:filter completed:^(BOOL completed) {
