...
- (void)setRecorder
{
    ...
    [_recorder recordForDuration:(NSTimeInterval)60.0f];
...
- (void)setSesstion
{
    ...
    if(_session == nil)
    {
        MMLog(@"Error creating session: %@", [sessionError description]);
    }
    else
    {
...
- (void)startRecord
{
    ...
    recordTime = 0;
    playTimer = [NSTimer scheduledTimerWithTimeInterval:0.2 target:self selector:@selector(countVoiceTime) userInfo:nil repeats:YES];
}
