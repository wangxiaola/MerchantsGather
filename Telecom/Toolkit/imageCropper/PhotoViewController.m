...
-(void)bottomView
{
    ...
    UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(1*(SCREEN_WIDTH - 100), 7, 80, 36)];
    [btn setTitle:@"确定" forState:UIControlStateNormal];
    [bottomView addSubview:btn];
    [btn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    btn.backgroundColor = NAVIGATION_COLOR;
    btn.layer.cornerRadius = 4;
    [btn addTarget:self action:@selector(buttonClicked) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:bottomView];
    
    // 旋转
    UIButton *rotateBtn =[[UIButton alloc] initWithFrame:CGRectMake(1*(SCREEN_WIDTH - 200), 7, 80, 36)];
    [rotateBtn setTitle:@"取消" forState:UIControlStateNormal];
    [rotateBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    rotateBtn.backgroundColor = NAVIGATION_COLOR;
    rotateBtn.layer.cornerRadius = 4;
    [bottomView addSubview:rotateBtn];
    [rotateBtn addTarget:self action:@selector(backBtn) forControlEvents:UIControlEventTouchUpInside];
